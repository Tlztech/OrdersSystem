...
public class Timesale24Sp {
	public void basicUpdate(String shop) throws Exception {
		...
		if ("trend777".equals(shop)) {
			file = new File("C:\\pages\\24timesale_trend.html");
		} else if ("coverforefront".equals(shop)) {
			file = new File("C:\\pages\\24timesale_cover.html");
		}
		...
		for (Item item : tomorrowItemList) {
			...
			url = (url.replace("https://image.rakuten.co.jp/",
					"https://thumbnail.image.rakuten.co.jp/@0_mall/") + "?_ex=220x220&s=2&r=1");
			shoriDiv = shoriDiv
					.replaceFirst("&\\{to_itemurl}", item.getItemUrl());
		...
		try {
			File f = new File("C:\\temp\\24timesale_" + shop + ".html");
			if (!f.exists()) {
				f.createNewFile();
			}
		...
		remote = "24timesale.html";
		local = ("C:\\temp\\24timesale_" + shop + ".html");

...
public class AUShop {
	...
	public List<String> getOrderNo() throws Exception {
		...
		String result = getHttpRsp(
				URL_GETORDERNOLIST + date + "&shopId" + shopId + "&orderStatus=" + URLEncoder.encode("発送待ち", "utf-8"));
		if (result.isEmpty()) {
	...
	public List<AUOrder> getOrders() throws Exception {
		...
		SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd");
		String date = sdf1.format(new Date());
		String result = getHttpRsp(URL_GETORDERINFOLIST + date + "&shopId=" + shopId + "&orderStatus="
				+ URLEncoder.encode("発送待ち", "utf-8"));
		if (result.isEmpty()) {
				...
				for (int i = 0; i < orderMapList.size(); i++) {
					...
					for (Map<String, Object> itemMap : itemMapList) {
						...
						if (Utility.isEmptyString((String) itemMap.get("itemManagementId"))) {
							...
							item.setItemCode(((String) itemMap.get("itemCode")).replaceAll("--", "-")
									+ ((String) itemMap.get("itemManagementId")).replaceAll("--", "-"));
						}
	...
	public void updateOrderStock(List<String> commodityIdList, List<String> detailNoList, List<String> quantityList)
			throws Exception {
		if (Utility.isEmptyList(commodityIdList) || Utility.isEmptyList(detailNoList)
				|| Utility.isEmptyList(quantityList) || (commodityIdList.size() != detailNoList.size())
				|| (commodityIdList.size() != quantityList.size()) || (detailNoList.size() != quantityList.size())) {
			MessageFromAU e = new MessageFromAU();
			e.setCode("");
			e.setMessage("更新データ不正。商品ID個数：" + (Utility.isEmptyList(commodityIdList) ? "0" : commodityIdList.size())
					+ ",個別ID個数：" + (Utility.isEmptyList(commodityIdList) ? "0" : commodityIdList.size()) + ",在庫数量個数："
					+ (Utility.isEmptyList(commodityIdList) ? "0" : commodityIdList.size()));
			messageFromAUList.add(e);
		} else {
			String result = postHttpRsp(URL_POSTSTOCKLIST,
					convertStockToXml(commodityIdList, detailNoList, quantityList));
			if (result.isEmpty()) {
				...
				if (resultMap.get("status").equals("1")) {
					if (resultMap.get("error") != null) {
						MessageFromAU e = new MessageFromAU();
						e.setCode((String) ((Map<String, Object>) resultMap.get("error"))
								.get("code"));
						e.setMessage((String) ((Map<String, Object>) resultMap.get("error"))
								.get("message"));
						messageFromAUList.add(e);
					} else {
						...
						for (int i = 0; i < updateResultList.size(); i++) {
							if (null == (Map<String, Object>) updateResultList.get(i).get("error")) {
								...
								MessageFromAU e = new MessageFromAU();
								e.setCode((String) ((Map<String, Object>) updateResultList.get(i).get("error"))
										.get("code"));
								e.setMessage("itemCode:" + (String) (updateResultList.get(i).get("itemCode")) + ","
										+ (String) ((Map<String, Object>) updateResultList.get(i).get("error"))
												.get("message"));
								messageFromAUList.add(e);
							}
						}
...
//		System.out.println(conn.getResponseCode());
	...
	private String convertStockToXml(List<String> commodityIdList, List<String> detailNoList,

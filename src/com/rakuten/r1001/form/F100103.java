package com.rakuten.r1001.form;

import java.io.Serializable;
import java.util.List;

public class F100103 implements Serializable {

	private static final long serialVersionUID = 1L;

	private String chumonshameiji = null;
	private String chumonshanamae = null;
	private String chumonshameijifurigana = null;
	private String chumonshanamaefurigana = null;
	private String chumonshayubinbango1 = null;
	private String chumonshayubinbango2 = null;
	private String chumonshajushotodofuken = null;
	private String chumonshajushotoshiku = null;
	private String chumonshajushochojijo = null;
	private String chumonshadenwabango1 = null;
	private String chumonshadenwabango2 = null;
	private String chumonshadenwabango3 = null;
	private String meruadoresu = null;
	private String oshiharaihoho = null;
	private String haisohoho = null;
	private String sofusakimeiji = null;
	private String sofusakinamae = null;
	private String sofusakimeijifurigana = null;
	private String sofusakinamaefurigana = null;
	private String sofusakiyubinbango1 = null;
	private String sofusakiyubinbango2 = null;
	private String sofusakijushotodofuken = null;
	private String sofusakijushotoshiku = null;
	private String sofusakijushochoijo = null;
	private String sofusakidenwabango1 = null;
	private String sofusakidenwabango2 = null;
	private String sofusakidenwabango3 = null;
	private String gokeishouhin = null;
	private String gokeizei = null;
	private String gokeisouryou = null;
	private String gokeidaibikiryou = null;
	private String seikyukingaku = null;
	private String resultcount = null;
	private String pointriyo = null;
	private String tuikashohinbango = null;
	private String sonotatesuryo = "0";
	private String sonotawaribikigaku = "0";
	private String sonotariyogaku = "0";

	/**
	 * @return the sonotatesuryo
	 */
	public String getSonotatesuryo() {
		return sonotatesuryo;
	}

	/**
	 * @param sonotatesuryo
	 *            the sonotatesuryo to set
	 */
	public void setSonotatesuryo(String sonotatesuryo) {
		this.sonotatesuryo = sonotatesuryo;
	}

	/**
	 * @return the sonotawaribikigaku
	 */
	public String getSonotawaribikigaku() {
		return sonotawaribikigaku;
	}

	/**
	 * @param sonotawaribikigaku
	 *            the sonotawaribikigaku to set
	 */
	public void setSonotawaribikigaku(String sonotawaribikigaku) {
		this.sonotawaribikigaku = sonotawaribikigaku;
	}

	private List<ShohinList> shohinList = null;

	/**
	 * @return the chumonshameiji
	 */
	public String getChumonshameiji() {
		return chumonshameiji;
	}

	/**
	 * @param chumonshameiji
	 *            the chumonshameiji to set
	 */
	public void setChumonshameiji(String chumonshameiji) {
		this.chumonshameiji = chumonshameiji;
	}

	/**
	 * @return the chumonshanamae
	 */
	public String getChumonshanamae() {
		return chumonshanamae;
	}

	/**
	 * @param chumonshanamae
	 *            the chumonshanamae to set
	 */
	public void setChumonshanamae(String chumonshanamae) {
		this.chumonshanamae = chumonshanamae;
	}

	/**
	 * @return the chumonshameijifurigana
	 */
	public String getChumonshameijifurigana() {
		return chumonshameijifurigana;
	}

	/**
	 * @param chumonshameijifurigana
	 *            the chumonshameijifurigana to set
	 */
	public void setChumonshameijifurigana(String chumonshameijifurigana) {
		this.chumonshameijifurigana = chumonshameijifurigana;
	}

	/**
	 * @return the chumonshanamaefurigana
	 */
	public String getChumonshanamaefurigana() {
		return chumonshanamaefurigana;
	}

	/**
	 * @param chumonshanamaefurigana
	 *            the chumonshanamaefurigana to set
	 */
	public void setChumonshanamaefurigana(String chumonshanamaefurigana) {
		this.chumonshanamaefurigana = chumonshanamaefurigana;
	}

	/**
	 * @return the chumonshayubinbango1
	 */
	public String getChumonshayubinbango1() {
		return chumonshayubinbango1;
	}

	/**
	 * @param chumonshayubinbango1
	 *            the chumonshayubinbango1 to set
	 */
	public void setChumonshayubinbango1(String chumonshayubinbango1) {
		this.chumonshayubinbango1 = chumonshayubinbango1;
	}

	/**
	 * @return the chumonshayubinbango2
	 */
	public String getChumonshayubinbango2() {
		return chumonshayubinbango2;
	}

	/**
	 * @param chumonshayubinbango2
	 *            the chumonshayubinbango2 to set
	 */
	public void setChumonshayubinbango2(String chumonshayubinbango2) {
		this.chumonshayubinbango2 = chumonshayubinbango2;
	}

	/**
	 * @return the chumonshajushotodofuken
	 */
	public String getChumonshajushotodofuken() {
		return chumonshajushotodofuken;
	}

	/**
	 * @param chumonshajushotodofuken
	 *            the chumonshajushotodofuken to set
	 */
	public void setChumonshajushotodofuken(String chumonshajushotodofuken) {
		this.chumonshajushotodofuken = chumonshajushotodofuken;
	}

	/**
	 * @return the chumonshajushotoshiku
	 */
	public String getChumonshajushotoshiku() {
		return chumonshajushotoshiku;
	}

	/**
	 * @param chumonshajushotoshiku
	 *            the chumonshajushotoshiku to set
	 */
	public void setChumonshajushotoshiku(String chumonshajushotoshiku) {
		this.chumonshajushotoshiku = chumonshajushotoshiku;
	}

	/**
	 * @return the chumonshajushochojijo
	 */
	public String getChumonshajushochojijo() {
		return chumonshajushochojijo;
	}

	/**
	 * @param chumonshajushochojijo
	 *            the chumonshajushochojijo to set
	 */
	public void setChumonshajushochojijo(String chumonshajushochojijo) {
		this.chumonshajushochojijo = chumonshajushochojijo;
	}

	/**
	 * @return the chumonshadenwabango1
	 */
	public String getChumonshadenwabango1() {
		return chumonshadenwabango1;
	}

	/**
	 * @param chumonshadenwabango1
	 *            the chumonshadenwabango1 to set
	 */
	public void setChumonshadenwabango1(String chumonshadenwabango1) {
		this.chumonshadenwabango1 = chumonshadenwabango1;
	}

	/**
	 * @return the chumonshadenwabango2
	 */
	public String getChumonshadenwabango2() {
		return chumonshadenwabango2;
	}

	/**
	 * @param chumonshadenwabango2
	 *            the chumonshadenwabango2 to set
	 */
	public void setChumonshadenwabango2(String chumonshadenwabango2) {
		this.chumonshadenwabango2 = chumonshadenwabango2;
	}

	/**
	 * @return the chumonshadenwabango3
	 */
	public String getChumonshadenwabango3() {
		return chumonshadenwabango3;
	}

	/**
	 * @param chumonshadenwabango3
	 *            the chumonshadenwabango3 to set
	 */
	public void setChumonshadenwabango3(String chumonshadenwabango3) {
		this.chumonshadenwabango3 = chumonshadenwabango3;
	}

	/**
	 * @return the meruadoresu
	 */
	public String getMeruadoresu() {
		return meruadoresu;
	}

	/**
	 * @param meruadoresu
	 *            the meruadoresu to set
	 */
	public void setMeruadoresu(String meruadoresu) {
		this.meruadoresu = meruadoresu;
	}

	/**
	 * @return the oshiharaihoho
	 */
	public String getOshiharaihoho() {
		return oshiharaihoho;
	}

	/**
	 * @param oshiharaihoho
	 *            the oshiharaihoho to set
	 */
	public void setOshiharaihoho(String oshiharaihoho) {
		this.oshiharaihoho = oshiharaihoho;
	}

	/**
	 * @return the haisohoho
	 */
	public String getHaisohoho() {
		return haisohoho;
	}

	/**
	 * @param haisohoho
	 *            the haisohoho to set
	 */
	public void setHaisohoho(String haisohoho) {
		this.haisohoho = haisohoho;
	}

	/**
	 * @return the sofusakimeiji
	 */
	public String getSofusakimeiji() {
		return sofusakimeiji;
	}

	/**
	 * @param sofusakimeiji
	 *            the sofusakimeiji to set
	 */
	public void setSofusakimeiji(String sofusakimeiji) {
		this.sofusakimeiji = sofusakimeiji;
	}

	/**
	 * @return the sofusakinamae
	 */
	public String getSofusakinamae() {
		return sofusakinamae;
	}

	/**
	 * @param sofusakinamae
	 *            the sofusakinamae to set
	 */
	public void setSofusakinamae(String sofusakinamae) {
		this.sofusakinamae = sofusakinamae;
	}

	/**
	 * @return the sofusakimeijifurigana
	 */
	public String getSofusakimeijifurigana() {
		return sofusakimeijifurigana;
	}

	/**
	 * @param sofusakimeijifurigana
	 *            the sofusakimeijifurigana to set
	 */
	public void setSofusakimeijifurigana(String sofusakimeijifurigana) {
		this.sofusakimeijifurigana = sofusakimeijifurigana;
	}

	/**
	 * @return the sofusakinamaefurigana
	 */
	public String getSofusakinamaefurigana() {
		return sofusakinamaefurigana;
	}

	/**
	 * @param sofusakinamaefurigana
	 *            the sofusakinamaefurigana to set
	 */
	public void setSofusakinamaefurigana(String sofusakinamaefurigana) {
		this.sofusakinamaefurigana = sofusakinamaefurigana;
	}

	/**
	 * @return the sofusakiyubinbango1
	 */
	public String getSofusakiyubinbango1() {
		return sofusakiyubinbango1;
	}

	/**
	 * @param sofusakiyubinbango1
	 *            the sofusakiyubinbango1 to set
	 */
	public void setSofusakiyubinbango1(String sofusakiyubinbango1) {
		this.sofusakiyubinbango1 = sofusakiyubinbango1;
	}

	/**
	 * @return the sofusakiyubinbango2
	 */
	public String getSofusakiyubinbango2() {
		return sofusakiyubinbango2;
	}

	/**
	 * @param sofusakiyubinbango2
	 *            the sofusakiyubinbango2 to set
	 */
	public void setSofusakiyubinbango2(String sofusakiyubinbango2) {
		this.sofusakiyubinbango2 = sofusakiyubinbango2;
	}

	/**
	 * @return the sofusakijushotodofuken
	 */
	public String getSofusakijushotodofuken() {
		return sofusakijushotodofuken;
	}

	/**
	 * @param sofusakijushotodofuken
	 *            the sofusakijushotodofuken to set
	 */
	public void setSofusakijushotodofuken(String sofusakijushotodofuken) {
		this.sofusakijushotodofuken = sofusakijushotodofuken;
	}

	/**
	 * @return the sofusakijushotoshiku
	 */
	public String getSofusakijushotoshiku() {
		return sofusakijushotoshiku;
	}

	/**
	 * @param sofusakijushotoshiku
	 *            the sofusakijushotoshiku to set
	 */
	public void setSofusakijushotoshiku(String sofusakijushotoshiku) {
		this.sofusakijushotoshiku = sofusakijushotoshiku;
	}

	/**
	 * @return the sofusakijushochoijo
	 */
	public String getSofusakijushochoijo() {
		return sofusakijushochoijo;
	}

	/**
	 * @param sofusakijushochoijo
	 *            the sofusakijushochoijo to set
	 */
	public void setSofusakijushochoijo(String sofusakijushochoijo) {
		this.sofusakijushochoijo = sofusakijushochoijo;
	}

	/**
	 * @return the sofusakidenwabango1
	 */
	public String getSofusakidenwabango1() {
		return sofusakidenwabango1;
	}

	/**
	 * @param sofusakidenwabango1
	 *            the sofusakidenwabango1 to set
	 */
	public void setSofusakidenwabango1(String sofusakidenwabango1) {
		this.sofusakidenwabango1 = sofusakidenwabango1;
	}

	/**
	 * @return the sofusakidenwabango2
	 */
	public String getSofusakidenwabango2() {
		return sofusakidenwabango2;
	}

	/**
	 * @param sofusakidenwabango2
	 *            the sofusakidenwabango2 to set
	 */
	public void setSofusakidenwabango2(String sofusakidenwabango2) {
		this.sofusakidenwabango2 = sofusakidenwabango2;
	}

	/**
	 * @return the sofusakidenwabango3
	 */
	public String getSofusakidenwabango3() {
		return sofusakidenwabango3;
	}

	/**
	 * @param sofusakidenwabango3
	 *            the sofusakidenwabango3 to set
	 */
	public void setSofusakidenwabango3(String sofusakidenwabango3) {
		this.sofusakidenwabango3 = sofusakidenwabango3;
	}

	/**
	 * @return the gokeishouhin
	 */
	public String getGokeishouhin() {
		return gokeishouhin;
	}

	/**
	 * @param gokeishouhin
	 *            the gokeishouhin to set
	 */
	public void setGokeishouhin(String gokeishouhin) {
		this.gokeishouhin = gokeishouhin;
	}

	/**
	 * @return the gokeizei
	 */
	public String getGokeizei() {
		return gokeizei;
	}

	/**
	 * @param gokeizei
	 *            the gokeizei to set
	 */
	public void setGokeizei(String gokeizei) {
		this.gokeizei = gokeizei;
	}

	/**
	 * @return the gokeisouryou
	 */
	public String getGokeisouryou() {
		return gokeisouryou;
	}

	/**
	 * @param gokeisouryou
	 *            the gokeisouryou to set
	 */
	public void setGokeisouryou(String gokeisouryou) {
		this.gokeisouryou = gokeisouryou;
	}

	/**
	 * @return the gokeidaibikiryou
	 */
	public String getGokeidaibikiryou() {
		return gokeidaibikiryou;
	}

	/**
	 * @param gokeidaibikiryou
	 *            the gokeidaibikiryou to set
	 */
	public void setGokeidaibikiryou(String gokeidaibikiryou) {
		this.gokeidaibikiryou = gokeidaibikiryou;
	}

	/**
	 * @return the seikyukingaku
	 */
	public String getSeikyukingaku() {
		return seikyukingaku;
	}

	/**
	 * @param seikyukingaku
	 *            the seikyukingaku to set
	 */
	public void setSeikyukingaku(String seikyukingaku) {
		this.seikyukingaku = seikyukingaku;
	}

	/**
	 * @return the resultcount
	 */
	public String getResultcount() {
		return resultcount;
	}

	/**
	 * @param resultcount
	 *            the resultcount to set
	 */
	public void setResultcount(String resultcount) {
		this.resultcount = resultcount;
	}

	/**
	 * @return the shohinList
	 */
	public List<ShohinList> getShohinList() {
		return shohinList;
	}

	/**
	 * @param shohinList
	 *            the shohinList to set
	 */
	public void setShohinList(List<ShohinList> shohinList) {
		this.shohinList = shohinList;
	}

	public String getPointriyo() {
		return pointriyo;
	}

	public void setPointriyo(String pointriyo) {
		this.pointriyo = pointriyo;
	}

	public String getTuikashohinbango() {
		return tuikashohinbango;
	}

	public void setTuikashohinbango(String tuikashohinbango) {
		this.tuikashohinbango = tuikashohinbango;
	}

	public String getSonotariyogaku() {
		return sonotariyogaku;
	}

	public void setSonotariyogaku(String sonotariyogaku) {
		this.sonotariyogaku = sonotariyogaku;
	}

}

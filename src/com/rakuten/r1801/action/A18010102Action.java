...
public class A18010102Action extends BaseAction {
	...
	protected void exec() throws Exception {
		...
		try {
			...
			sql = "SELECT * FROM SHOP WHERE SITE = ? AND SHOP_ID = ? AND COMPANY_ID = ? AND DELETE_FLG is null";
			ps = conn.prepareStatement(sql);
			ps.setString(1, f180101.getPlatformSelect());
			ps.setString(2, f180101.getShopSelect());
//			ps.setInt(3, Integer.parseInt(f180101.getCompanyIdSelect()));
			ps.setInt(3, f180101.getCompanyIdSelect());
			...
			while (rs.next()) {
				...
				f180101.setCompanyId(rs.getInt("COMPANY_ID"));
	...
	protected void isValidated() throws Exception {
		if (0 == (f180101.getCompanyIdSelect())) {
			addError(null, "请选择公司");
		}
		if ("0".equals(f180101.getPlatformSelect())) {
			addError(null, "请选择平台");
		}
		if ("0".equals(f180101.getShopSelect())) {
			addError(null, "请选择店铺");
		}
	}

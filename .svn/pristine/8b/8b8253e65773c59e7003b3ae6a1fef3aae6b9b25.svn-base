...
public class A07020104Action extends BaseAction {
	...
	protected void exec() throws Exception {
		...
		updateCommodity(commodity_idArr, kosuArr, shubetuIdArr, remarksArr);

	}

	private void updateCommodity(String[] commodity_idArr, String[] kosuArr,
			String[] shubetuIdArr, String[] remarksArr) throws Exception {
		...
		try {
			...
			for (int i = 0; i < commodity_idArr.length; i++) {
				...
				sql = "UPDATE TBL00015 SET COMMODITY_STATUS = ? , RECIEVE_QUANTITY = ? , REMARKS = ? WHERE COMMODITY_ID = ? AND SHUBETU_ID = ?";
				ps = conn.prepareStatement(sql);
				ps.setString(1, "1");
